...
int read_next_line(int fd, char** row) {
    ...
    while (str == NULL || !strchr(str, '\n')) {
        ...
        tmp        = str;  // 기존 문자열 저장
        str        = tmp ? malloc(strlen(tmp) + strlen(buff) + 1) : strdup(buff);
        if (!str) {
            terminate(MEMORY_ALLOCATION);
        }
        if (tmp) {
...
static bool has_cmd_str_format(t_str_const row, t_str_const cmd_str) {
    return str_eq(row, cmd_str)
        && is_delimiter((char)row.ptr[cmd_str.len]);
}

// is_command: 주어진 row가 NAME_CMD_STRING 또는 COMMENT_CMD_STRING인지 확인
bool is_command(t_pars* pars, char* row) {
    static const t_str_const name_cmd    = literal_str(NAME_CMD_STRING);
    static const t_str_const comment_cmd = literal_str(COMMENT_CMD_STRING);
    static const usize       max_len     = name_cmd.len < comment_cmd.len ? comment_cmd.len : name_cmd.len;

    const t_str_const self = str_const((u8*)(row + pars->col), max_len);

    return has_cmd_str_format(self, name_cmd)
        || has_cmd_str_format(self, comment_cmd);
}
// bool is_command(t_pars* pars, char* row) {
//     return (
//         (!strncmp(row + pars->col, NAME_CMD_STRING, strlen(NAME_CMD_STRING))
//          && is_delimiter(row[pars->col + strlen(NAME_CMD_STRING)]))
//         || (!strncmp(row + pars->col, COMMENT_CMD_STRING, strlen(COMMENT_CMD_STRING)) && is_delimiter(row[pars->col + strlen(COMMENT_CMD_STRING)]))
//     );
// }
...
int upd_buffer(t_pars* pars) {
    ...
    void* temp = realloc(pars->code, pars->code_size);
    if (!temp) {
        terminate(MEMORY_ALLOCATION);
    }
    pars->code = temp;
